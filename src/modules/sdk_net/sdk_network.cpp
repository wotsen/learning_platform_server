...
static bool get_host_addr(const Ip4Addr &src, const Ip4Addr &dest, struct SdkNetInterface &sdk_interface);
...
bool SdkServer::tcp_connection_made(void)
{
	...
	tcp_srv_->onConnCreate([&] {
		...
		con->onRead([&](TcpConnPtr con) {
			...
			Sdk req;
			Sdk res;
			struct SdkNetInterface sdk_interface;
